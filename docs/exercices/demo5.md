...
## Ressources Graphes (Chapitre 14)

Implémentation Java de différentes représentations de graphes, avec un guide détaillé pour comprendre les choix de conception et les compromis :

* [Guide détaillé de l'implémentation](demo5-graphes-implementation.md)
...
??? question "Question 8 — La classe Stack de Java"
    ...
    ??? success "Réponse"
        **Faux.** `java.util.Stack` est une classe **legacy** (héritée de Java 1.0) qui pose plusieurs problèmes :
        ...
        1. Elle hérite de `Vector`, une API legacy qui n'est pas centrée sur la sémantique LIFO
...
??? question "Question 9 — Deque comme structure universelle"
    Un `Deque` (double-ended queue) peut simuler efficacement à la fois une `Stack` et une `Queue`.

    ??? success "Réponse"
        **Vrai.** Un `Deque` permet les insertions et suppressions aux **deux extrémités** en O(1), ce qui suffit pour implémenter les deux structures :

        | Structure | Opération | Implémentation avec Deque |
        |-----------|-----------|---------------------------|
        ...
        Toutes ces opérations sont **O(1)** avec un `ArrayDeque`.

        C'est d'ailleurs pourquoi la Javadoc de Java recommande `ArrayDeque` à la fois pour les piles (`push`/`pop`) et les files (`offer`/`poll`) :

        ```java
        Deque<String> stack = new ArrayDeque<>();  // Comme pile
        Deque<String> queue = new ArrayDeque<>();  // Comme file
        ```

        **Point clé :** Le `Deque` est une **généralisation** de la pile et de la file. Si une structure permet des opérations aux deux bouts, elle peut se restreindre à un seul bout (pile) ou utiliser un bout pour l'entrée et l'autre pour la sortie (file).

---

#### Bloc C — Files et Priorités

??? question "Question 10 — Position du 3e plus petit dans un min-heap"
    Dans un min-heap de n éléments (n ≥ 7), **sans répétition de clés/priorités**, le 3e plus petit élément se trouve nécessairement à la profondeur 1 (c'est-à-dire un enfant de la racine).

    ??? success "Réponse"
        **Faux.** Le 3e plus petit élément peut se trouver à la profondeur 1 **ou** à la profondeur 2.

        (Hypothèse utilisée : priorités toutes distinctes, comme dans l'énoncé.)

        L'invariant du heap garantit que chaque parent ≤ ses enfants. Donc :

        - Le **1er minimum** est à la racine (profondeur 0)
        - Le **2e minimum** est l'un des enfants de la racine (profondeur 1)
        - Le **3e minimum** peut être :
            - L'**autre enfant** de la racine (profondeur 1), **ou**
            - Un **petit-enfant** de la racine (profondeur 2), si les deux enfants de la racine sont grands

        ```
        Exemple où le 3e minimum est à la profondeur 2 :

              1               Tableau : [1, 10, 5, 20, 15, 2, 8]
            /   \
          10     5
         / \   / \
        20 15 2   8    ← 2 est le 3e plus petit, à la profondeur 2 !
        ```

        **Règle générale :** Le k-ième plus petit élément d'un min-heap peut se trouver à n'importe quelle profondeur de 0 à k−1.

        **Piège classique :** Croire qu'un heap est « presque trié ». La seule garantie est parent ≤ enfants, pas de relation entre frères.

??? question "Question 11 — Parcours préfixe d'un min-heap"
    Un parcours préfixe (preorder) d'un min-heap liste toujours les clés en ordre non-décroissant.

    ??? success "Réponse"
        **Faux.** Un parcours préfixe visite : racine, puis sous-arbre gauche, puis sous-arbre droit. La propriété de heap (parent ≤ enfants) ne garantit **pas** un ordre non-décroissant lors d'un parcours préfixe.
        ...
        Parcours préfixe : 1, 3, 5, 4, 2
        ```

        L'ordre obtenu est 1, 3, 5, 4, **2** — ce n'est pas non-décroissant car 2 < 4 et 2 < 5.

        **Pourquoi ?** Un heap garantit seulement que chaque parent ≤ ses enfants (relation **verticale**). Il n'y a aucune garantie entre :

        - les frères (3 vs 2)
        - les nœuds de sous-arbres différents (5 vs 2)

        C'est un **BST** (arbre binaire de recherche) qui donnerait un parcours infixe trié, pas un heap avec un parcours préfixe.

        **Piège classique :** Confondre la propriété de heap avec celle d'un BST. Un heap n'est **pas** trié — il est seulement « partiellement ordonné ».

??? question "Question 12 — Position du deuxième minimum dans un heap"
...
??? question "Question 13 — Heap vs arbre binaire de recherche"
    ...
    ??? success "Réponse"
        **Faux.** Les propriétés d'ordre sont fondamentalement différentes :
        ...
        | Minimum | Racine (O(1)) | Nœud le plus à gauche (O(h)) |
        | Recherche par clé | O(n) | O(h), donc O(log n) si équilibré |
...
??? question "Question 14 — Arbres et graphes"
...
??? question "Question 15 — Matrice vs liste d'adjacence"
...
??? question "Question 16 — Nombre maximum d'arêtes"
...
??? question "Question 17 — Liste d'arêtes et voisinage"
...
??? question "Question 18 — Nombre maximum d'arêtes avec composantes connexes"
    Un graphe simple non-orienté avec 12 sommets et 3 composantes connexes peut avoir au maximum 45 arêtes.

    ??? success "Réponse"
        **Vrai.** Pour maximiser le nombre d'arêtes avec exactement k composantes connexes, il faut **concentrer les sommets** dans une seule composante et laisser les autres aussi petites que possible (1 sommet chacune).

        Avec n = 12 sommets et k = 3 composantes :

        - Mettre **10 sommets** dans une composante (graphe complet) : C(10, 2) = 10 × 9 / 2 = **45 arêtes**
        - Mettre **1 sommet** dans chacune des 2 autres composantes : **0 arête** chacune

        Total maximum = **45 arêtes** ✓

        **Pourquoi cette répartition est optimale ?** Si on répartit plus uniformément, par exemple (4, 4, 4) :

        - C(4,2) × 3 = 6 × 3 = **18 arêtes** — bien moins que 45 !

        **Formule générale :** Pour n sommets et k composantes, le maximum d'arêtes est C(n−k+1, 2) = (n−k+1)(n−k) / 2.

        Vérification : (12−3+1)(12−3) / 2 = 10 × 9 / 2 = 45 ✓

        **Piège classique :** Penser que répartir uniformément maximise les arêtes. C'est l'inverse — la fonction C(n, 2) est convexe, donc concentrer les sommets produit plus d'arêtes.
...
??? question "Question 5 — Taille d'une pile et d'une file après opérations"
    Une pile `S` a subi les opérations suivantes : 25 `push`, 12 `top`, 10 `pop` (dont 3 ont retourné `null` car la pile était vide).
    ...
    ??? success "Réponse"
        **B) 18**
        ...
        Analysons l'effet de chaque opération sur la taille :

        | Opération | Effet sur la taille | Occurrences | Total |
        |-----------|---------------------|-------------|-------|
        | `push` | +1 | 25 | +25 |
        | `top` | 0 (consultation) | 12 | 0 |
        | `pop` réussi | −1 | 7 | −7 |
        | `pop` sur pile vide | 0 (retourne null) | 3 | 0 |

        Taille = 25 − 7 = **18**

        **Points clés :**

        - `top()` ne modifie pas la pile — c'est une consultation
        - Les 3 `pop` qui retournent `null` n'ont rien retiré (la pile était vide à ce moment)
        - Seuls les 10 − 3 = **7** `pop` réussis ont retiré un élément

        **Piège classique :** Compter les 10 `pop` au lieu de 7. Les `pop` sur pile vide ne changent pas la taille !
...
??? question "Exercice 3 — Trace complète d'une file à priorité"
    On exécute les opérations suivantes sur une file à priorité min (min-heap) initialement vide. Pour chaque `removeMin()`, indiquez la paire (clé, valeur) retournée.
    ...
    ??? success "Réponse"
        Traçons l'état du heap après chaque opération :

        | # | Opération | Heap (tableau, min en tête) | Retour |
        |---|-----------|---------------------------|--------|
        | 1 | `insert(5,A)` | `[(5,A)]` | — |
        ...
        **Détail de l'étape 12** (la plus intéressante — up-heap) :

        ```
        Avant insert(2,H) :     Après up-heap :
              6                        2
            /   \                    /   \
           8     7                  6     7
                                  /
                                 8
        insert(2,H) à la fin → [6, 8, 7, 2]
        2 < 8 (parent) → swap → [6, 2, 7, 8]
        2 < 6 (parent) → swap → [2, 6, 7, 8]
        ```

        **Point clé :** Notez que les removeMin ne retournent **pas** les éléments dans l'ordre d'insertion. L'insertion de (2,H) tard dans la séquence fait qu'elle est retournée **avant** (6,L) qui était insérée plus tôt.

??? question "Exercice 4 — Représentations d'un graphe orienté (Chap. 14 + code)"
    Cet exercice reprend exactement le graphe orienté de `docs/files/code/graphs/src/Main.java`
    (méthode `demonstrateFullGraph`) :
    ...
    * Arcs étiquetés :
    ...
    Utilisez l'ordre des sommets **`[a, b, c]`** pour la matrice d'adjacence.

    **Questions** :

    1. Donnez la `edge list` exacte (avec étiquettes).
    2. Donnez la matrice d'adjacence (3×3), avec `1` si l'arc existe, sinon `0`.
    3. Donnez la liste d'adjacence des arcs **sortants** (`outgoing`) et **entrants** (`incoming`) pour chaque sommet.
    4. Donnez `outDegree`, `inDegree` et `degree` de chaque sommet.
    5. Donnez le résultat des appels : `getEdge(a,b)`, `getEdge(b,a)`, `getEdge(a,c)`, `getEdge(c,c)`.
    ...
    ??? success "Réponse"
        ...
        **2) Matrice d'adjacence (ordre `[a,b,c]`) :**
        ...
        Lecture : `M[i][j] = 1` s'il existe un arc `i → j`.

        ---

        **3) Listes d'adjacence sortantes et entrantes :**
        ...
        **4) Degrés :**
        ...
        | Sommet | `outDegree` | `inDegree` | `degree = out + in` |
        |--------|-------------|------------|----------------------|
        | `a` | 1 | 1 | 2 |
        | `b` | 2 | 1 | 3 |
        | `c` | 1 | 2 | 3 |

        Vérification digraphe :
        `Σ outDegree = 1 + 2 + 1 = 4 = |E|`
        `Σ inDegree  = 1 + 1 + 2 = 4 = |E|`

        ---

        **5) Résultats de `getEdge` :**

        * `getEdge(a,b)` retourne `α`
        * `getEdge(b,a)` retourne `β`
        * `getEdge(a,c)` retourne `null`
        * `getEdge(c,c)` retourne `δ`

        Point important : en orienté, `getEdge(u,v)` n'est pas équivalent à `getEdge(v,u)`.
    ...
    **Questions** :
    ...
    ??? success "Réponse"
        **1. Lister toutes les liaisons — Liste d'arêtes :**
        ...
        Dans le modèle du manuel (section 14.2), on peut aussi exposer `edges()` en **O(m)** pour les autres représentations en maintenant une collection globale des arêtes.

        Sans cette collection auxiliaire, on retombe sur :

        - Liste d'adjacence : O(n + m) avec gestion des doublons
        - Matrice : O(n²) par scan de la matrice
...
??? question "Scénario 2 - Système hospitalier multi-structures"
    **Contexte (niveau architecture)** : Un hôpital régional avec 8 départements doit gérer trois sous-systèmes informatiques. L'hôpital reçoit environ 200 patients par jour.

    **Sous-système A — File d'attente générale :**
    Les patients arrivent à l'accueil et sont traités dans l'ordre d'arrivée (premier arrivé, premier servi). Environ 30 patients attendent en permanence.

    **Sous-système B — Urgences :**
    Les patients urgents sont traités selon leur score de sévérité (1 = mineur, 10 = critique). Parfois, l'état d'un patient change pendant l'attente et son score doit être mis à jour. Environ 15 patients en attente.

    **Sous-système C — Réseau de départements :**
    Les 8 départements sont reliés par 12 couloirs (bidirectionnels). On doit fréquemment trouver les départements voisins d'un département donné (pour transférer un patient).

    **Questions** :

    1. Pour chaque sous-système (A, B, C), quelle structure de données est la plus adaptée ? Justifiez avec les complexités.
    2. Donnez un mini tableau des opérations dominantes et de leurs coûts cibles pour A, B et C.
    3. Le sous-système C est-il dense ou creux ? Quelle représentation principale choisir ?
    4. L'hôpital veut aussi produire un rapport quotidien « liste complète des couloirs ». Proposez une organisation des données pour bien supporter à la fois `incidentEdges(v)` et l'itération globale des arêtes.

    ??? success "Réponse"
        **1. Structures adaptées :**

        | Sous-système | Structure | Justification |
        |-------------|-----------|---------------|
        | **A** — File générale | **Queue (ArrayDeque)** | FIFO pur : `enqueue` O(1), `dequeue` O(1). L'ordre d'arrivée détermine l'ordre de traitement. |
        | **B** — Urgences | **File à priorité (heap)** | Extraction du plus urgent en O(log n), insertion en O(log n). Avec n ≈ 15, log₂(15) ≈ 4 — très rapide. |
        | **C** — Réseau | **Liste d'adjacence** | `incidentEdges(v)` en O(deg(v)) pour les transferts. Graphe creux → efficace en mémoire. |

        ---

        **2. Coûts cibles (opérations dominantes) :**

        | Sous-système | Opération dominante | Coût visé |
        |-------------|---------------------|-----------|
        | A — Accueil | `enqueue`, `dequeue` | O(1), O(1) |
        | B — Urgences | `insert`, `removeMax` | O(log n), O(log n) |
        | C — Réseau | `incidentEdges(v)` | O(deg(v)) |

        ---

        **3. Dense ou creux ?**

        - n = 8 départements, m = 12 couloirs
        - Maximum d'arêtes : n(n−1)/2 = 8×7/2 = **28**
        - Ratio : 12/28 ≈ **43%** → graphe de **densité moyenne**

        Pour ce petit graphe, les trois représentations sont raisonnables, mais la **liste d'adjacence** est le meilleur choix :

        | Représentation | `incidentEdges(v)` | Espace |
        |---------------|-------------------|--------|
        | Liste d'arêtes | O(m) = O(12) | O(n + m) = 20 |
        | **Liste d'adjacence** | **O(deg(v))** ≈ O(3) | O(n + m) = 20 |
        | Matrice d'adjacence | O(n) = O(8) | O(n²) = 64 |

        L'opération dominante (trouver les voisins pour les transferts) est O(deg(v)) ≈ O(3) avec la liste d'adjacence, plus efficace que O(8) avec la matrice ou O(12) avec la liste d'arêtes.

        ---

        **4. Rapports globaux + voisinage local :**

        Maintenir **deux vues synchronisées** du graphe C :

        - Vue principale : **liste d'adjacence** (voisinage rapide pour transferts)
        - Vue auxiliaire : **liste d'arêtes** (rapport global des couloirs en O(m))

        Compromis :

        - Lecture locale (`incidentEdges(v)`) très efficace
        - Lecture globale (`edges()`) directe
        - Coût supplémentaire : maintenir la cohérence entre les deux vues lors des ajouts/retraits d'arêtes

??? question "Scénario 3 — Gestion d'une salle d'urgences"
    **Contexte (niveau opérationnel)** : On zoome sur le sous-système des urgences uniquement. La file contient environ 50 patients.

    **Règles métier** :

    - Priorité principale : sévérité (10 avant 1)
    - Bris d'égalité : à sévérité égale, ordre d'arrivée (FIFO)
    - Opérations : `insert`, `removeMax`, `replaceKey`, `remove(entry)`

    **Questions** :

    1. Donnez une clé de priorité qui implémente ces règles (sévérité + FIFO).
    2. Mini-trace : on insère successivement `(Alice,4,t1)`, `(Bob,7,t2)`, `(Chloé,7,t3)`, `(David,3,t4)`. Quels sont les deux prochains patients extraits ?
    3. Ensuite, David passe de 3 à 9. Donnez la complexité de cette mise à jour avec un heap simple, puis avec une file à priorité adaptable.
    4. Bob quitte la salle d'attente avant traitement. Quelle opération utiliser et quelle complexité (heap simple vs adaptable) ?
    5. Comparez heap adaptable vs liste triée pour cette charge : 40 insertions, 30 extractions, 15 mises à jour de priorité, 5 retraits ciblés (n≈50).

    ??? success "Réponse"
        **1. Clé de priorité :**

        Utiliser une clé lexicographique :

        - clé primaire : `severity` (descendant)
        - clé secondaire : `arrivalTime` (ascendant)

        Donc `(7,t2)` est prioritaire sur `(7,t3)` car `t2 < t3`.

        ---

        **2. Mini-trace :**

        File après insertions :

        ```java
        (Bob,7,t2), (Chloé,7,t3), (Alice,4,t1), (David,3,t4)
        ```

        Les deux prochains extraits sont :

        1. **Bob** (sévérité 7, arrivé avant Chloé)
        2. **Chloé** (sévérité 7)

        ---

        **3. Mise à jour de David (3 → 9) :**

        - **Heap simple** : rechercher David O(n), puis reheap O(log n) → **O(n)**
        - **Heap adaptable** : `replaceKey(entryDavid, nouvelleClé)` → **O(log n)**

        ---

        **4. Bob quitte la file :**

        - **Heap simple** : trouver Bob O(n), puis supprimer/reheap O(log n) → **O(n)**
        - **Heap adaptable** : `remove(entryBob)` → **O(log n)**

        ---

        **5. Charge mixte (n≈50, log₂(n)≈6) :**

        Approximation du coût total :

        - **Heap adaptable** : `(40 + 30 + 15 + 5) × log n ≈ 90 × 6 = 540`
        - **Liste triée** :
            - insertions : `40 × 50 = 2000`
            - extractions : `30 × 1 = 30`
            - mises à jour : `15 × 50 = 750`
            - retraits ciblés : `5 × 50 = 250`
            - total ≈ `3030`

        Le heap adaptable est environ **5 à 6×** plus efficace sur cette charge.



??? question "Scénario 4 - Réseau de distribution d'énergie"
    ...
    **Question 4** : Détection de connexité après panne
    ...
    ??? success "Réponse"
        **Question 1 : Choix de représentation**
        ...
        | Op. 2 : toutes les lignes (`edges()`) | **O(m)** = O(1200) | **O(m)** = O(1200) | **O(m)** = O(1200) |
        | Op. 3 : retirer une ligne (`removeEdge(e)`) | O(1)* | O(1)* | O(1) |
        ...
        *Notes (manuel 14.2)* :
        - Le coût O(m) de `edges()` suppose une collection globale des arêtes maintenue en plus de la représentation principale.
        - Le coût O(1) pour retirer une ligne suppose qu'on possède déjà une référence vers l'arête (`removeEdge(e)`).
...
??? question "Exercice A — Représentations d'un graphe"
    Soit le graphe **non-orienté** G défini par la liste d'adjacence suivante :
    ...
    **Questions** :
    ...
    2. Donnez la **matrice d'adjacence** de G (5×5).
    3. Donnez la **liste d'arêtes** (edge list) de G.
    4. Calculez le **degré** de chaque sommet. Vérifiez votre réponse avec le nombre total d'arêtes.
    5. Ce graphe est-il **dense** ou **creux** ? Justifiez.
    ...
    ??? success "Réponse"
        **1. Dessin du graphe :**
        ...
        (A-B, A-C, A-E, B-C, B-D, C-D, C-E — 7 arêtes)
        ...
        **Remarque :** La matrice est **symétrique** car le graphe est non-orienté (M[i][j] = M[j][i]).
        ...
        **Vérification** (théorème des poignées de main) :
        ...
        **5. Dense ou creux ?**
        ...
        - Nombre d'arêtes : m = 7
        - Maximum possible (graphe simple non-orienté) : n(n−1)/2 = 5×4/2 = **10**
        - Ratio : 7/10 = **70%**

        Ce graphe est relativement **dense** (70% des arêtes possibles sont présentes).

        Pour ce type de graphe, la matrice d'adjacence est un choix raisonnable car :

        - L'espace « gaspillé » est faible (seulement 3 zéros sur 10 paires)
        - La vérification d'adjacence est O(1) au lieu de O(deg)
        - Mais pour seulement 5 sommets, la différence pratique est négligeable
...
│   └─► Queue (ArrayDeque ou LinkedList)
│
...
| `java.util.Stack` = choix moderne pour LIFO | Classe legacy : préférer `Deque` / `ArrayDeque` |
...
| Liste d'adjacence | O(V + E) | O(1) | O(min(deg(u),deg(v))) | O(deg) | **O(E)** |
| Matrice d'adjacence | O(V²) | O(1) | O(1) | O(V) | **O(E)** |

*Pour `Lister toutes arêtes`: hypothèse du manuel (Table 14.1) où une collection globale des arêtes est maintenue.*
